        ...
        for epoch in range(args.epochs):
            ...
            for batch_idx, batch in enumerate(tqdm(loader)):
                ...
                if epoch == 0:
                    ...
                    # 计算当前批次与所有训练样本的相似度
                    classwise_sim = torch.einsum('b d, n d -> b n', all_feats[uq_idxs], all_feats)  # (batch_size, dataset_size)
                    _, indices = classwise_sim.topk(k=args.k+1, dim=-1, largest=True, sorted=True)
                    indices = indices[:, 1:]  # 排除自身

                    # 提取 kNN 特征
                    knn_feats = all_feats[indices]  # (batch_size, k, feature_dim)
                    ...
                    # 注意力权重：点积 + 缩放 + Softmax
                    d_k = Q.size(-1)  # 特征维度
                    attention_logits = torch.einsum('b q d, b k d -> b q k', Q, K) / (d_k**0.5)  # (batch_size, 1, k)
                    attention_weights = torch.softmax(attention_logits, dim=-1)  # (batch_size, 1, k)

                    # 加权求和
                    weighted_knn_feats = torch.einsum('b q k, b k d -> b q d', attention_weights, V)  # (batch_size, 1, feature_dim)
            ...
            for i in range(len(num_clusters)):
                ...
                wandb.log({
                    "epoch": epoch,
                    f"cluster_{i}_train_old_acc": old_acc_train,
                    f"cluster_{i}_test_all_acc": all_acc_test,
                    f"cluster_{i}_test_old_acc": old_acc_test,
                    f"cluster_{i}_test_new_acc": new_acc_test,
    ...
    with torch.no_grad():
        for epoch in range(args.epochs):
            ...
            for i in range(len(num_clusters)):
                ...
                wandb.log({
                    "epoch": epoch,
                    f"val_cluster_{i}_old_acc": old_acc_val,
                    f"test_cluster_{i}_all_acc": all_acc_test,
                    f"test_cluster_{i}_old_acc": old_acc_test,
                    f"test_cluster_{i}_new_acc": new_acc_test,
                })
...
if __name__ == "__main__":
    ...
    print(args)
    args.model_name = "/home/beichen_zhang/zhenghao/cl/CMS/log1/cms/log/(16.12.2024_|_00.300)/checkpoints/model_best.pt"
