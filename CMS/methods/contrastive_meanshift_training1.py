...
def train(model, train_loader, test_loader, train_loader_memory, args):
    ...
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')  # 确保设备一致
    ...
    for epoch in range(args.epochs):
        ...
        with torch.no_grad():
            ...
            for batch_idx, batch in enumerate(tqdm(train_loader_memory)):
                images, _, _, _ = batch
                images = torch.cat(images, dim=0).to(device)  # 将图像移到GPU
                features = model(images)
                all_feats.append(features.detach())  # 直接存储GPU上的特征
            all_feats = torch.cat(all_feats).to(device)  # 确保所有特征在GPU上
        ...
        for batch_idx, batch in enumerate(tqdm(train_loader)):
            ...
            images = torch.cat(images, dim=0).to(device)  # 图像移到GPU

            # 计算当前批次的特征
            features = model(images)  # 输出的features在device上
            ...
            # 获取对应的k个近邻特征
            knn_feats = all_feats[indices]  # 近邻特征仍在GPU上

            # 将权重调整为3D张量
            weights = normalized_sim.unsqueeze(-1)  # shape: (batch_size * n_views, k, 1)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--exp_root', type=str, default='/home/beichen_zhang/zhenghao/cl/CMS/log1')
