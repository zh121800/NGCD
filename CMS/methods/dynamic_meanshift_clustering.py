        ...
        for epoch in range(args.epochs):
            ...
            for batch_idx, batch in enumerate(tqdm(loader)):
                ...
                if epoch == 0:
                    ...
                    indices = indices[:, 1:]  # 排除自身（第一个点积最大值）

                    knn_feats = all_feats[indices]  # k 近邻特征
                    topk_sim = torch.gather(classwise_sim, 1, indices)  # k 近邻相似度

                    # 使用Softmax将相似度转换为正值且归一化
                    normalized_sim = torch.softmax(topk_sim, dim=-1)
                    weights = normalized_sim.unsqueeze(-1)

                    # 计算加权特征
                    weighted_knn_feats = (knn_feats * weights).sum(dim=1)
            ...
            for i in range(len(num_clusters)):
                ...
                all_acc_test, old_acc_test, new_acc_test = log_accs_from_preds(
                    y_true=u_targets, y_pred=preds[~mask_lab], mask=mask,
        ...
        for epoch in range(args.epochs):
            ...
            for batch_idx, batch in enumerate(tqdm(loader)):
                ...
                if epoch == 0:
                    ...
                    topk_sim = torch.gather(classwise_sim, 1, indices)
                    normalized_sim = topk_sim / topk_sim.sum(dim=-1, keepdim=True)
                    weights = normalized_sim.unsqueeze(-1)
                    weighted_knn_feats = (knn_feats * weights).sum(dim=1)
                    new_feats[uq_idxs] = (1 - args.alpha) * all_feats[uq_idxs] + args.alpha * weighted_knn_feats.detach().cpu()
            ...
            for batch_idx, batch in enumerate(tqdm(val_loader)):
                ...
                if epoch == 0:
                    ...
                    current_feats = all_feats_val[start_idx:start_idx+len(uq_idxs)]

                    # 计算当前批次与所有验证集样本的相似度
                    classwise_sim = torch.einsum('b d, n d -> b n', current_feats, all_feats_val)
                    _, indices = classwise_sim.topk(k=args.k + 1, dim=-1, largest=True, sorted=True)
                    indices = indices[:, 1:]  # 排除自身

                    # 获取K近邻特征
                    knn_feats = all_feats_val[indices]
                    topk_sim = torch.gather(classwise_sim, 1, indices)
                    ...
                    # 计算加权K近邻特征
                    weighted_knn_emb_val = (knn_feats * weights).sum(dim=1)

                    # 更新特征
                    new_feats_val[start_idx:start_idx+len(uq_idxs)] = (
                        (1 - args.alpha) * current_feats + args.alpha * weighted_knn_emb_val.detach().cpu()
                    )
            ...
            for i in range(len(num_clusters)):
                ...
                preds = fcluster(linked, t=threshold, criterion='distance')
                all_acc_test, old_acc_test, new_acc_test = log_accs_from_preds(
                    y_true=targets, y_pred=preds, mask=mask_cls, T=epoch, eval_funcs=args.eval_funcs,
                    save_name='IMS test ACC', print_output=True)
...
if __name__ == "__main__":
    ...
    print(args)
    args.model_name = "/home/beichen_zhang/zhenghao/cl/CMS/log/cms/checkpoints/model_best.pt"
